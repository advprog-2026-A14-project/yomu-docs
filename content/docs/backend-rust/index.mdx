...
Clean Architecture is a design philosophy proposed by Robert C. Martin (Uncle Bob) that organizes code into concentric circles, with the **Dependency Rule** as its cornerstone: **dependencies must always point inward**. Inner layers know nothing about outer layers.

```mermaid
graph TD
    subgraph OUTER["🔷 Presentation Layer (Delivery Mechanism)"]
        P1[Axum Controllers]
        P2[gRPC Handlers]
        P3[Middleware / Filters]
    end
    subgraph APP["🟢 Application Layer (Business Orchestration)"]
        A1[Use Cases / Interactors]
        A2[DTOs — Request / Response]
        A3[Application Services]
    end
    subgraph DOM["🔴 Domain Layer (Core Business Logic)"]
        D1[Entities & Aggregates]
        D2[Value Objects]
        D3[Repository Ports — Traits]
        D4[Domain Errors — thiserror]
    end
    subgraph INF["🟡 Infrastructure Layer (Technical Detail)"]
        I1[PostgreSQL Repositories]
        I2[Redis Cache Adapters]
        I3[HTTP Clients — reqwest]
        I4[External Service Clients]
    end
...
**The Dependency Rule** dictates that:
- **Domain** knows nothing about HTTP, SQL, JSON, or frameworks
- **Application** knows about Domain, but not about Axum or PostgreSQL
- **Infrastructure** implements the Ports (traits) defined by Domain
- **Presentation** depends on Application to execute use cases

This inversion of dependencies means our business logic is **framework-agnostic**. We can replace Axum with Actix-web, SQLx with Diesel, or PostgreSQL with SQLite — without touching a single line of domain or application code.
