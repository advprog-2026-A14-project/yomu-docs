...
sequenceDiagram
    ...
    RUST-->>API: (user_id, score, clan_name)
    API->>JAVA: POST /auth/users/by-ids
    API->>JAVA: (ids array) [...] }
    JAVA-->>API: (user_id, username, display_name)
    API->>API: Gabung: leaderboard_entries
    API-->>FE: { leaderboard: [ { user_id, score, username, rank } ] }
```
