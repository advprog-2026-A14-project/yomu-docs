---
title: Forum Diskusi
description: Backend forum Java - komentar artikel, nested replies, reaction toggle, dan integrasi frontend BFF
---

Modul forum menyediakan diskusi per artikel melalui komentar berjenjang, edit/delete komentar, dan reaction toggle. Implementasinya berada di package `forum` pada backend Java dan dipakai frontend melalui route handler BFF Next.js.
...
```mermaid
graph TD
    FE["Next.js page<br/>/forums/[articleId]"] --> BFF["Next.js BFF<br/>/api/v1/forums/*"]
    BFF --> CC["CommentController<br/>/api/v1/forums"]
...
Endpoint tulis mengambil user dari `CurrentUser`, sehingga token JWT wajib valid. Frontend tidak mengirim token dari JavaScript client; BFF membaca cookie `yomu_access_token` lalu meneruskan header `Authorization: Bearer <token>` ke backend Java.
</Callout>
...
| `src/app/api/v1/forums/**/route.ts` | BFF route handlers yang proxy ke Java |

Alur request:

```mermaid
sequenceDiagram
    Browser->>Next.js BFF: GET /api/v1/forums/{articleId}/comments
    Next.js BFF->>Java: GET /api/v1/forums/{articleId}/comments
    Java->>Next.js BFF: ApiResponse<Comment[]>
    Next.js BFF->>Browser: ApiResponse<Comment[]>

    Browser->>Next.js BFF: POST /api/v1/forums/comments/{commentId}/reactions
    Next.js BFF->>Next.js BFF: Baca cookie yomu_access_token
    Next.js BFF->>Java: POST + Authorization Bearer token
    Java->>Next.js BFF: ApiResponse<ReactionResponse>
    Next.js BFF->>Browser: ApiResponse<ReactionResponse>
```
